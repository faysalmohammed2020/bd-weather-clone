...
//     lat = lat.map((val) => (typeof val === "number" ? val : Number.parseFloat(val))).filter((val) => !isNaN(val))
//     lon = lon.map((val) => (typeof val === "number" ? val : Number.parseFloat(val))).filter((val) => !isNaN(val))
...
//     const hasLatDim = dims.some((dim) => ["lat", "latitude", "LAT", "LATITUDE", "y", "Y", "lats"].includes(dim))
//     const hasLonDim = dims.some((dim) => ["lon", "longitude", "LON", "LONGITUDE", "x", "X", "lons"].includes(dim))
...
//                 </div>
//                 <div>
//                   <h1 className="text-4xl font-bold text-white mb-2">NetCDF Visualizer</h1>
//                   <p className="text-xl text-blue-100">Advanced scientific data visualization and analysis platform</p>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                       {modalContent.title}
//                     </DialogTitle>
//                     <DialogDescription className="text-left">{modalContent.message}</DialogDescription>
//                   </DialogHeader>
//                   <DialogFooter>
//                     <Button
...
//                   </div>
//                 </div>
//                 <h3 className="text-xl font-semibold text-gray-800 mb-2">Processing NetCDF File</h3>
//                 <p className="text-gray-600 mb-4 text-center max-w-md">
//                   Analyzing your data and generating visualizations. This may take a few moments.
//                 </p>
//               </div>
//             </CardContent>
//           </Card>
...
function NetCDFVisualizer() {
  ...
  const processFileLocally = async () => {
    ...
    try {
      ...
      if (version === 2) {
        setModalContent({
          title: t("modal.netcdf4.title"),
          message: t("modal.netcdf4.message"),
          type: "warning",
        })
        setShowVersionModal(true)
        setIsProcessing(false)
        ...
        setModalContent({
          title: t("modal.netcdf3.title"),
          message: t("modal.netcdf3.message"),
          type: "success",
        })
        setShowVersionModal(true)
        setIsProcessing(false)
        ...
        setModalContent({
          title: t("modal.unsupported.title", { version }),
          message: t("modal.unsupported.message", { version }),
          type: "warning",
        })
        setShowVersionModal(true)
        setIsProcessing(false)
  ...
  const getPlotLayout = (): Partial<Layout> => {
    ...
    switch (plotType) {
      ...
      default:
        return {
          ...baseLayout,
          xaxis: { title: { text: t("visualization.axis.index") }, gridcolor: "#f0f0f0" },
          yaxis: { title: { text: selectedVariable }, gridcolor: "#f0f0f0" },
        }
    }
  }
  ...
  const plotTypes = [
    {
      type: "line",
      icon: LineChart,
      label: t("visualization.types.line.label"),
      description: t("visualization.types.line.description"),
    },
    {
      type: "scatter",
      icon: ScatterChart,
      label: t("visualization.types.scatter.label"),
      description: t("visualization.types.scatter.description"),
    },
    {
      type: "histogram",
      icon: BarChart3,
      label: t("visualization.types.histogram.label"),
      description: t("visualization.types.histogram.description"),
    },
    {
      type: "heatmap",
      icon: MapIcon,
      label: t("visualization.types.heatmap.label"),
      description: t("visualization.types.heatmap.description"),
    },
    {
      type: "contour",
      icon: BoxSelect,
      label: t("visualization.types.contour.label"),
      description: t("visualization.types.contour.description"),
    },
    {
      type: "surface",
      icon: Box,
      label: t("visualization.types.surface.label"),
      description: t("visualization.types.surface.description"),
    },
  ]
    ...
    if (!fileData.images || Object.keys(fileData.images).length === 0) {
      return (
        <div className="flex items-center justify-center h-96 text-gray-500 bg-gray-50 rounded-lg">
          <div className="text-center">
            <ImageIcon className="h-12 w-12 mx-auto mb-2 text-gray-400" />
            <p className="text-lg font-medium">{t("spatial.noVisualizations")}</p>
            <p className="text-sm text-gray-400 mt-1">{t("spatial.description")}</p>
          </div>
        </div>
      )
    }
          ...
          <Card className="mb-8 shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            <CardContent className="py-12">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 mb-4 text-center max-w-md">{t("loading.description")}</p>
              </div>
            </CardContent>
          </Card>
        )}
                  ...
                  <TabsContent value="interactive" className="mt-6">
                    {ncData ? (
                      <div className="space-y-6">
                          ...
                          <CardContent>
                            ...
                            {plotType === "map" && !hasSpatialDimensions() && (
                              <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                                <div className="flex items-center gap-2 text-amber-800">
                                  <AlertCircle className="h-4 w-4" />
                                  <span className="text-sm font-medium">{t("visualization.mapRequirement")}</span>
                                </div>
                              </div>
                            )}
                          </CardContent>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                          ...
                          <div className="lg:col-span-1">
                            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
                              <CardHeader className="pb-4">
                                <CardTitle className="text-lg">{t("variables.title")}</CardTitle>
                              </CardHeader>
                              <CardContent>
                                <div className="space-y-2 max-h-96 overflow-y-auto">
                                  {Object.keys(ncData.variables).map((varName) => (
                              ...
                              <CardContent>
                                {selectedVariable ? (
                                  ...
                                  <div className="flex items-center justify-center h-96 text-gray-500 bg-gray-50 rounded-lg">
                                    <div className="text-center">
                                      <Database className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                                      <p className="text-lg font-medium">{t("variables.selectToVisualize")}</p>
                                      <p className="text-sm text-gray-400 mt-1">{t("variables.selectDescription")}</p>
                                    </div>
                                  </div>
                                )}
                              </CardContent>
                        ...
                        <Tabs defaultValue="export" className="space-y-6">
                          ...
                          <TabsContent value="metadata">
                            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
                              <CardHeader className="pb-4">
                                <CardTitle className="text-lg">{t("metadata.fileMetadata")}</CardTitle>
                              </CardHeader>
                              <CardContent className="space-y-6">
                                {/* NetCDF Version */}
                                {ncData.metadata.version && (
                                  <div>
                                    <h4 className="font-semibold mb-3 text-gray-800">{t("metadata.version")}</h4>
                                    <div className="bg-gray-50 p-4 rounded-lg border">
                                      <div className="flex items-center gap-2">
                                        {ncData.metadata.version === 1 ? (
                                          <CheckCircle className="h-5 w-5 text-green-600" />
                                ...
                                <div>
                                  <h4 className="font-semibold mb-3 text-gray-800">{t("metadata.dimensions")}</h4>
                                  <div className="bg-gray-50 p-4 rounded-lg border">
                                    <pre className="text-sm overflow-x-auto text-gray-700">
                                      {JSON.stringify(ncData.metadata.dimensions, null, 2)}
                                    </pre>
                                ...
                                <div>
                                  <h4 className="font-semibold mb-3 text-gray-800">{t("metadata.globalAttributes")}</h4>
                                  <div className="bg-gray-50 p-4 rounded-lg border max-h-64 overflow-y-auto">
                                    <pre className="text-sm text-gray-700">
                                      {JSON.stringify(ncData.metadata.globalAttributes, null, 2)}
                                    </pre>
                                ...
                                {selectedVariable && (
                                  <div>
                                    <h4 className="font-semibold mb-3 text-gray-800">
                                      {t("metadata.variableAttributes", { variable: selectedVariable })}
                                    </h4>
                                    <div className="bg-gray-50 p-4 rounded-lg border max-h-64 overflow-y-auto">
                                      <pre className="text-sm text-gray-700">
                                        {JSON.stringify(ncData.variables[selectedVariable].attributes, null, 2)}
                      ...
                      <div className="flex items-center justify-center h-96 text-gray-500 bg-gray-50 rounded-lg">
                        <div className="text-center">
                          <Activity className="h-12 w-12 mx-auto mb-4 text-gray-400" />
                          <p className="text-lg font-medium">{t("states.interactivePlots")}</p>
                          <p className="text-sm text-gray-400 mt-1">{t("states.uploadDescription")}</p>
                        </div>
                      </div>
                    )}
                  </TabsContent>
          ...
          <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            <CardContent className="py-16">
              <div className="text-center text-gray-500">
                ...
                <h3 className="text-2xl font-semibold mb-2 text-gray-800">{t("states.readyToAnalyze")}</h3>
                <p className="text-gray-600 max-w-md mx-auto">{t("states.readyDescription")}</p>
              </div>
            </CardContent>
          </Card>
        )}

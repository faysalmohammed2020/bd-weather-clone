    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const windSpeeds = todayWeatherObservations
          ...
          .filter((val) => !isNaN(val));
        if (windSpeeds.length > 0) {
          measurements[9] = (windSpeeds.reduce((a, b) => a + b, 0) / windSpeeds.length).toFixed(2);
        }
...
<<<<<<< HEAD
        ...
        const cloudAmounts = todayWeatherObservations
          ...
          .filter((val) => !isNaN(val));
        if (cloudAmounts.length > 0) {
          measurements[13] = (cloudAmounts.reduce((a, b) => a + b, 0) / cloudAmounts.length).toFixed(2);
        }
        ...
        const ranges = [
          "14-18", "19-23", "24-26", "27-28", "30-32", "33-35", "36-39", "40-42",
          "43-45", "46-48", "49-50", "51-53", "54-55", "56", "57-59", "60-63"
        ];

        const units = [
          "hPa", "hPa", "°C", "°C", "°C", "°C", "mm", "°C", "%", "m/s",
          "16Pts", "m/s", "16Pts", "oktas", "km", "H-M"
        ];

        const categories = [
          "pressure", "pressure", "temperature", "temperature", "temperature", "temperature",
          "precipitation", "temperature", "humidity", "wind", "wind", "wind", "wind",
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {Object.keys(groupedData).map((category) => {
              ...
              return (
                <Card key={category} className={`overflow-hidden border ${style.border} shadow-sm hover:shadow-md transition-shadow`}>
                        ...
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <span className={`flex items-center gap-1 px-2 py-1 rounded-md font-medium ${style.bg} ${style.text}`}>
                                {style.icon}
                                {item.label}
                              </span>
                            </div>
